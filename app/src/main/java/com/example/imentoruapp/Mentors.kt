...
class Mentors : Fragment() {
    ...
    override fun onCreateView(
        ...
        val provinceValues = arrayOf(
            "select province", "Eastern Cape", "Free State", "Gauteng", "KwaZulu-Natal",
            "Limpopo", "Mpumalanga", "Northern Cape", "North West", "Western Cape", "Other country"
        )
        setUpSpinner(provinces, provinceValues) { selectedProvince = it }

        val language = view.findViewById<Spinner>(R.id.language)
        val languageTypes = arrayOf(
            "Select your home language", "Afrikaans", "English", "isiNdebele", "isiXhosa",
            "isiZulu", "Sepedi", "Sesotho", "Setswana", "siSwati", "Tshivenda", "Xitsonga", "Other"
        )
        ...
        val interestTypes = arrayOf(
            "Select program you want to be part of it", "Career Mentorship", "Educational Support",
            "Community Services Support", "Language Learning",
            "STEMA(Sciences, Technology, Engineering, Mathematics & Agriculture) Education", "Sports & Recreation"
        )
        setUpSpinner(interest, interestTypes) { selectedInterest = it }

        val found = view.findViewById<Spinner>(R.id.found)
        val foundTypes = arrayOf(
            "Select how did you find us", "Facebook", "Twitter", "Instagram", "Linkedin",
            "Whatsapp groups or individual", "Internet (Searching/browsing)", "YouTube",
            "Word of mouth", "Radio interviews", "Others"
        )
        ...
        val submitButton = view.findViewById<LinearLayout>(R.id.submit)
        submitButton.setOnClickListener {
            val formData = mapOf(
                "id_number" to idNumber.text.toString(),
                "first_name" to name.text.toString(),
    ...
    private inner class SubmitFormTask : AsyncTask<Map<String, String>, Void, String>() {
        override fun doInBackground(vararg params: Map<String, String>): String {
            ...
            val url = URL("http://192.168.0.105/will_project/php/addMentor.php")
